...
combo_province = ttk.Combobox(
    ...
    width=20,
    state='readonly')
combo_province['values'] = data_province
combo_province.place(x=40, y=40)
combo_province.current(0)
combo_province.bind('<<ComboboxSelected>>', func_comb_selected)

combo_city = ttk.Combobox(
    top_win,
    width=20,
    state='readonly')
...
combo_city.current(0)
combo_city.bind('<<ComboboxSelected>>', func_comb_selected)


def refresh_lbl():
    p = combo_province.get()
    c = combo_city.get()
    lbl_info['text'] = 'The Position is %s %s ' % (p, c)
    return

p = combo_province.get()
c = combo_city.get()

lbl_info = tk.Label(top_win, width=280,
                    height=20,
                    anchor='nw',
                    text='choose one province please'
                    )
lbl_info['text'] = 'The Position is %s %s ' % (p, c)
lbl_info.place(x=440, y=85)
